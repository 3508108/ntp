...
## 9) Моніторинг і статистика DNS (актуально)
Розгорнуто синхронно на `fra-dns` і `fra-dns-2`.

### Скрипти
- `/usr/local/sbin/dns-health-check.js`
  - Перевірка DNS по `127.0.0.1` (A) і `::1` (AAAA)
  - Логує `status`, `answer_count`, `latency_ms` у `/var/log/named/health.log`
- `/usr/local/sbin/dns-alert-evaluator.js`
  - Аналізує останні health-check результати
  - Рахує послідовні помилки, формує події в `/var/log/named/alerts.log`
  - Підтримує Telegram-алерти через `/etc/dns-monitor.env` (опційно)
- `/usr/local/sbin/dns-stats-aggregate.js`
  - Генерує статистику `24h` і `7d`
  - Записує файли:
    - `/var/log/named/stats/latest.json`
    - `/var/log/named/stats/24h.json`
    - `/var/log/named/stats/7d.json`
  - Додає агреговані записи в `/var/log/named/stats.log`
...
### Поточний стан після налаштування
- Усі таймери `active` на обох серверах.
- `alerts.log`: події `action=none`, `consecutive_fails=0` (нормальний стан).
- `stats/24h.json`: `uptime_percent=100`, без fail на момент перевірки.
